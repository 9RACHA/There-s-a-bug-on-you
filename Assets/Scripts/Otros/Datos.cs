...
public class Datos {
    ...
    private void AlmacenarPokemonEnPC(PokemonModelo pokemon)
    {
    ...
    public bool SacarPokemonDelPC(PokemonModelo pokemon)
    {
        if (HayEspacioEnElEquipo())
        {
            _pokemonAlmacenadosEnPC.Remove(pokemon);
            AniadirPokemonAlEquipo(pokemon);
            return true;
        }
        else
    ...
    public bool DejarPokemonDelEquipoEnPC(PokemonModelo pokemon)
    {
        ...
        {
            EliminarPokemonDelEquipo(pokemon);
            _pokemonAlmacenadosEnPC.Add(pokemon);
            return true;
        }
    }

    private void EliminarPokemonDelEquipo(PokemonModelo pokemon)
    {
        for (int i = 0; i < _equipoPokemon.Length; i++)
        {
            if(_equipoPokemon[i].IdentificardorUnico == pokemon.IdentificardorUnico)
            {
